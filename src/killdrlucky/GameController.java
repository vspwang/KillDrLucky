...
public class GameController {
  ...
  /**
   * Constructs a {@code GameController} instance responsible for managing user
  ...
  public void playGame() throws IOException {
    ...
    out.append("Welcome to Kill Doctor Lucky!\n");
    ...
    while (model.getPlayers().isEmpty()) {
      ...
      if (cmd.startsWith("add")) {
        ...
        out.append("Please add at least one player before starting.\n");
      }
    }

    out.append("\nPlayers added. Game begins!\n");
    ...
    while (!model.isGameOver() && scan.hasNextLine() && turnCount < maxTurns) {
      ...
      if (current.isComputerControlled()) {
        out.append("\n==============================\n");
        out.append(" AI Turn — ").append(current.getName()).append("\n");
        out.append("==============================\n");
        ...
        out.append("[AI Action] ").append(result).append("\n");

        // Show state after the action
        out.append("After action:\n").append(model.describePlayer(current.getName())).append("\n");

        endTurn = true; // AI always ends its turn
      } else {
  ...
  private void printHelp() throws IOException {
    out.append("""
        \nAvailable commands:
        add <name> <startIndex> <isAi>   - Add player (AI or human)
        move <name> <direction>          - Move the player
        ...
        save <filename>                  - Save world image (mock)
        quit                             - End the game
        """);
  }
}
